...
public:
  Renderer(
    ...
    , const SDL_RendererFlags flags = static_cast<SDL_RendererFlags>(SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC));
  ~Renderer() = default;
...
public:
  ...
  auto RenderTexture(SDL_Texture *texture, const SDL_Rect *source, const SDL_Rect *destination) const -> void;
  auto SetDrawColor(const SDL_Color &color) const -> void;
  auto LoadImageFromJpg(const std::string &pathToFile) const -> SDL_Texture *;
  auto TextToTexture(TTF_Font *font, const std::string &text, const SDL_Color &color) const -> SDL_Texture *;
